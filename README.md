...
- リリースビルド: CI `build` ジョブが pinned BtbN GPL ffmpeg を `src-tauri/resources/ffmpeg/` へ配置してバンドル (Windows: NSIS / Linux: deb + AppImage)。ローカルでビルドする場合も同様に配置 (GPL表記必須, §11)。AppImage は linuxdeploy同梱patchelfが同梱ffmpegを書換え不可のため、ffmpeg抜きでビルド→ `packaging/appimage/inject-ffmpeg.sh` で後注入する方式。Arch系の導入手順は `docs/arch.md`
