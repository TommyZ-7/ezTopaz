...
- リリースビルド: CI `build` ジョブが pinned BtbN GPL ffmpeg を `src-tauri/resources/ffmpeg/` へ配置してバンドル (Windows: NSIS / Linux: deb)。ローカルでビルドする場合も同様に配置 (GPL表記必須, §11)。AppImage は同梱 ffmpeg への linuxdeploy patchelf 失敗のため保留 (§13.2 からの逸脱, CI コメント参照)。Arch系は `eztopaz-bin` (AUR, debリパック) を想定 — 手順は `docs/arch.md`
